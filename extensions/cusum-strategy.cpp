...
NFD_LOG_INIT("CusumStrategy");
NFD_REGISTER_STRATEGY(CusumStrategy);


const time::milliseconds CusumStrategy::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds CusumStrategy::RETX_SUPPRESSION_MAX(250);

// std::cout << ns3::Simulator::GetContext() << std::endl;


CusumStrategy::CusumStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    BOOST_THROW_EXCEPTION(std::invalid_argument("CusumStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    BOOST_THROW_EXCEPTION(std::invalid_argument(
      "CusumStrategy does not support version " + to_string(*parsed.version)));
  }
...
const Name&
CusumStrategy::getStrategyName()
{
  static Name strategyName("ndn:/localhost/nfd/strategy/cusum-strategy/%FD%05");
  // cout << m_id << endl;
  return strategyName;
}
...
void
CusumStrategy::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                         const shared_ptr<pit::Entry>& pitEntry)
{
  // int temp = ns3::Simulator::GetContext();
  // mycout << ns3::Simulator::Now().ToDouble (ns3::Time::S) << " " << temp << " " << interest << endl;
  int id = ns3::Simulator::GetContext();
  m_name = ns3::Names::FindName(ns3::NodeList::GetNode(id));
  auto name = interest.toUri().substr(0, 21);
  if(m_prefixwindow.size() >= Window){
...
void
CusumStrategy::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                     const shared_ptr<pit::Entry>& pitEntry)
{

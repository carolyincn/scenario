...
data = read.table("beta.txt", header=T)
df <- melt(data, id.var = c("beta"))
names(df) <- c('beta', 'Type', 'rate')

pdf("beta.pdf", width=8, height=4)
g.root <- ggplot(df) +
  geom_point(aes (x=beta, y=rate, color=Type, shape = Type), size=2) +
  geom_line(aes (x=beta, y=rate, color=Type), size=0.5) +
  # scale_x_discrete() + 

...
df = read.table("h.txt", header=T)

pdf("h.pdf", width=8, height=2)
g.root <- ggplot(df) +
  geom_point(aes (x=h, y=delay16), size=2) +
  geom_line(aes (x=h, y=delay16), size=0.5) +
...
g.root <- ggplot(df) +
  geom_point(aes (x=h, y=FP), size=2) +
  geom_line(aes (x=h, y=FP), size=0.5) +
  # scale_x_continuous(breaks=seq(0, 180, 30)) +
  ylab("Rate") +
  # scale_colour_discrete(labels=c("FP", "FN")) +
  # scale_shape_discrete(labels=c("FP", "FN")) +
